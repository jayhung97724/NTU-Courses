...
void delline(line** line, char c) {
    ...
    else if (c == 'L') lowerLine = (*line)->next;
    else if (c == 'U') upperLine = (*line)->next;
}
...
double intersect(line* line1, line* line2, char c) {
    ...
    else if (abs(det) <= ERROR && x == 0 && y == 0) return 1;
    else return 0;
}
...
int main() {
    ...
    while (num_LowerBound + num_UpperBound > 2 ) {
        ...
        if (lower_y <= upper_y && lowerSlope.min <= lowerSlope.max && lowerSlope.max < 0) {
            x_LB = x_Me;
        }

        if (lower_y <= upper_y && lowerSlope.max >= lowerSlope.min && lowerSlope.min > 0) {
            x_RB = x_Me;
        }
        ...
        if (lower_y > upper_y && lowerSlope.max >= upperSlope.min && lowerSlope.min <= upperSlope.max) {
            printf("NA\n");
            return 0;
        }
    ...
    // 左界右界有交集，上下界各一條，討論斜率與截距
        ...
        if (lowerLine->m * upperLine->m > 0 && abs(upperLine->m) > abs(lowerLine->m)){
        ...
        if (abs(lowerLine->m - upperLine->m) < ERROR && lowerLine->t > upperLine->t){
            printf("NA\n");
            return 0;
        }
        if (abs(lowerLine->m - upperLine->m) < ERROR  && lowerLine->t <= upperLine->t){
    ...
    // 左界右界有交集，上界兩條，討論斜率與截距
        ...
        if (lowerLine->m * lowerLine->next->m < 0){
            if (intersect(lowerLine, lowerLine->next, 'x') < x_LB){
                printf("%d\n", (int)round(max(lowerLine->y_value(x_LB), lowerLine->next->y_value(x_LB))));
                return 0;
            }
            else if (intersect(lowerLine, lowerLine->next, 'x') > x_RB){
                printf("%d\n", (int)round(max(lowerLine->y_value(x_RB), lowerLine->next->y_value(x_RB))));
                return 0;
            }
            else if (x_LB < intersect(lowerLine, lowerLine->next, 'x') < x_RB){
                printf("%d\n", (int)round(lowerLine->y_value(intersect(lowerLine, lowerLine->next, 'x'))));
                return 0;
            }
            else {
                printf("-INF\n");
